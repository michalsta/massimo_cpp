...
public:
    ...
    std::string to_cpp_string() const {
        ...
        result += "N_minimal=" + std::to_string(N_minimal) + ", ";
        ...
        result += "], frame_probs=[";
        for (const auto &prob : frame_probs) {
            result += std::to_string(prob) + ", ";
        }
        result += "], scan_indices=[";
        for (const auto &index : scan_indices) {
            result += std::to_string(index) + ", ";
        }
        result += "], scan_probs=[";
        for (const auto &prob : scan_probs) {
            result += std::to_string(prob) + ", ";
        }
        result += "], tof_indices=[";
        for (const auto &index : tof_indices) {
            result += std::to_string(index) + ", ";
        }
        result += "], tof_probs=[";
        for (const auto &prob : tof_probs) {
            result += std::to_string(prob) + ", ";
        }
        result += "])";
        return result;
